...
#[tokio::test]
async fn test_count_voter_weight_with_invalid_registrar_in_subsequent_call_error(
) -> Result<(), TransportError> {
    // Arrange
    let mut nft_voter_test = NftVoterTest::start_new().await;
    ...
    // Try use registrar from different realm
    ...
    // Assert
    assert_nft_voter_err(err, NftVoterError::InvalidRegistrarForVoterWeightCounter);
