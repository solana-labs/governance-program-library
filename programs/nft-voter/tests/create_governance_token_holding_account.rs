...
async fn test_create_governance_token_holding_account() -> Result<(), TransportError> {
    ...
    assert_eq_formatted(
        0,
        governance_token_holding_account_cookie.account.delegated_amount,
        "delegated_amount",
    );
    assert_eq_formatted(
        COption::None,
        governance_token_holding_account_cookie.account.close_authority,
        "close_authority",
    );
    assert_eq_formatted(
...
#[tokio::test]
async fn test_create_governance_token_holding_account_nft_is_not_part_of_configured_collection_errors() -> Result<(), TransportError> {
    // Arrange
    let mut nft_voter_test = NftVoterTest::start_new().await;
    ...
    let nft_collection_cookie = nft_voter_test.token_metadata.with_nft_collection().await?;
    let max_voter_weight_record = nft_voter_test.with_max_voter_weight_record(&registrar_cookie).await?;
    assert_eq!(0, registrar_cookie.account.collection_configs.len(), "Unexpected collection already configured for registrar");
    nft_voter_test.with_collection(&registrar_cookie, &nft_collection_cookie, &max_voter_weight_record, None).await?;
    let registrar_updated = nft_voter_test.get_registrar_account(&registrar_cookie.address).await;
    assert_eq!(1, registrar_updated.collection_configs.len(), "Unable to add collection to registrar");
...
#[tokio::test]
async fn test_create_governance_token_holding_account_already_exists_errors() -> Result<(), TransportError> {
    // Arrange
    let mut nft_voter_test = NftVoterTest::start_new().await;
