...
async fn test_update_voter_weight_record() -> Result<(), TransportError> {
    ...
    let nft1_cookie = nft_voter_test
        .token_metadata
        .with_nft_v2(&registrar_cookie.collection_cookies.as_ref().unwrap().first().unwrap(), &voter_cookie, None)
        .await?;
...
async fn test_update_voter_weight_record_with_no_nft_error() -> Result<(), TransportError> {
    ...
    let nft1_cookie = nft_voter_test
        .token_metadata
        .with_nft_v2(
            &registrar_cookie.collection_cookies.as_ref().unwrap().first().unwrap(),
            &voter_cookie,
            Some(CreateNftArgs {
                amount: 0,
                ..Default::default()
