...
async fn test_resize_registrar() -> Result<(), TransportError> {
    ...
    // Act
    let registrar_cookie_resized = token_voter_test.with_resize_registrar(&realm_cookie, 2).await?;
...
async fn test_resize_registrar_with_configured_mint() -> Result<(), TransportError> {
    ...
    // Act
    let err = token_voter_test.with_resize_registrar(&realm_cookie, 2).await
        .err()
        .unwrap();
    assert_token_voter_err(err, TokenVoterError::InvalidResizeMaxMints);

    let registrar_cookie_resized = token_voter_test.with_resize_registrar(&realm_cookie, 3).await?;
    ...
    assert_eq!(registrar.max_mints, registrar_cookie_resized.account.max_mints);
