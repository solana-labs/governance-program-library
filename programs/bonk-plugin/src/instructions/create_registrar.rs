...
use crate::{
  error::BonkPluginError, state::*, 
  utils::stake_pool::StakePool, 
...
pub fn create_registrar_handler(ctx: Context<CreateRegistrar>) -> Result<()> {
  ...
  if let Some(
    previous_voter_weight_plugin_program_info
  ) = 
    &ctx.accounts.previous_voter_weight_plugin_program_id 
  {
    registrar.previous_voter_weight_plugin_program_id = Some(previous_voter_weight_plugin_program_info.key());
  }
