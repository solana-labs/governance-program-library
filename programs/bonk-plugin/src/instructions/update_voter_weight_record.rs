...
pub fn update_voter_weight_record_handler<'info>(
    ...
    require_eq!(stake_receipts_count, receipts_len, BonkPluginError::ReceiptsCountMismatch);
    ...
    voter_weight = voter_weight.checked_add(input_voter_weight_record.get_voter_weight()).unwrap();

    let new_deposit_len = stake_deposit_record.new_deposit_len(
        stake_receipts_count, 
        action_target, 
        action
    );
    ...
    if voter_weight_record.weight_action_target == Some(action_target) &&
        voter_weight_record.weight_action == Some(action)
    {        
        voter_weight = voter_weight.checked_sub(stake_deposit_record.previous_voter_weight).unwrap();
