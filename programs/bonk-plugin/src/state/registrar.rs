use {
  super::VoterWeightAction, crate::{
    error::BonkPluginError, id, state::VoterWeightRecord, utils::stake_deposit_receipt:: StakeDepositReceipt, SPL_TOKEN_STAKING_PROGRAM_ID
  }, 
  anchor_lang::prelude::*, 
...
pub fn resolve_stake_deposit_weight(
  ...
  require_gt!(u8::MAX, unique_stake_deposit_receipts.len() as u8, BonkPluginError::MaximumDepositsReached);
  ...
  let stake_deposit_receipt: StakeDepositReceipt = StakeDepositReceipt::deserialize(stake_deposit_receipt_buf)?;
  ...
  if action == VoterWeightAction::CastVote {
    if let Some(proposal_info) = proposal_info {
      require_keys_eq!(proposal_info.key(), action_target, BonkPluginError::ActionTargetMismatch);

      let stake_deposit_end_time = stake_deposit_receipt.deposit_timestamp
      .checked_add(stake_deposit_receipt.lockup_duration as i64).unwrap();
  
      let proposal_end_time = resolve_proposal_end_time(
        registrar, 
        proposal_info, 
        governance_info
      )?;
...
pub fn resolve_proposal_end_time(
  ...
  let proposal_end_time = proposal.voting_base_time_end(&governance.config)
